...
// 사용법 - throttle(원래 쓰려고했던 함수, 주기)
// import debounce from "lodash/debounce"  // 특정 이벤트의 마지막만 실행하도록 설정 (ex: 아이디 입력검사)
// 사용법 - debounce(원래 쓰려고했던 함수, 주기)
...
const BookInfinite = (props)=>{
    ...
    const loadBook = ()=>{
        loading.current =true; //로딩중으로 설정(순서가 보장되어야 하므로 Ref를 사용 )
        axios({
        ...
        .then(response=>{
            ...
            loading.current = false; //로딩완료로 설정
        });
    };
    ...
    //다음페이지
    // state를 설정할때 함수를 여러개쓴다면 누가 먼저 실행지는 랜덤이다 -> 따라서 useEffet로 해야한다
    const nextPage = ()=>{
